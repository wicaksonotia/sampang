$(document).ready(function () {
    $('#tgl_search').datepicker({
        ...
        autoclose: true,
    }).on('changeDate', prosesSearchPrauji);
...
//        aktifitas = 'Proses prauji dengan ipad dengan id hasil ' + selr + ' kode TL ' + kirim + kdlain;
        ...
        $.messager.confirm('Confirm', 'Apakah anda yakin ingin memproses kendaraan berikut?', function (r) {
            if (r) {
                var form_data = new FormData(document.getElementById("formUpload"));
                $.ajax({
                    type: 'POST',
                    url: 'Prauji/UploadImage',
                    data: form_data,
                    contentType: false,
                    cache: false,
                    processData: false,
                    ...
                    success: function (data) {
                        hidelargeloader();
                        uploadImage(urlAct, id_hasil_uji, kirim, posisi, username);
                        $('#formUpload').each(function () {
                            this.reset();
                        });
                        $('#replace_file').load('Prauji/ReplaceFile');
                        return false;
                    },
                    error: function () {
                        hidelargeloader();
...
function uploadImage(urlAct, id_hasil_uji, kirim, posisi, username) {
    $.ajax({
        type: 'POST',
        url: urlAct,
