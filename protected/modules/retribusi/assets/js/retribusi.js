$(document).ready(function () {
  ...
  $("#cobalah").html(
    "<button type='button' class='disabled btn btn-warning'>" +
      "<span class='glyphicon glyphicon-picture'></span> There is no image scan</button>"
  );
});
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data != 0) {
        ...
        if (data.img_scan !== "") {
          $("#cobalah").html(
            "<button type='button' class='btn btn-warning' onclick=\"cobalah('" +
              data.img_scan +
              "')\"><span class='glyphicon glyphicon-picture'></span> Click Here! to show image scan</button>"
          );
        } else {
          $("#cobalah").html(
            "<button type='button' class='disabled btn btn-warning'>" +
        ...
        $("#FORM_ID_KEND").val(0);
        $("#FORM_JBB").val(0);
        $("#show_img_scan_true").hide();
        $("#show_img_scan_false").show();
      }
    },
    error: function (data) {
      $("#tidak_ada").hide();
...
function cobalah(params) {
  var image = new Image();
  image.src = "data:image/jpg;base64," + params;
...
function prosesSubmit(urlAct, idForm) {
  ...
  var form_data = new FormData(document.getElementById("formPendaftaran"));
  $.ajax({
    url: urlAct,
    type: "POST",
    data: form_data,
    ...
    success: function (data) {
      ...
      if (data.ada == "true") {
        ...
        $("#formPendaftaran").each(function () {
          this.reset();
        });
        $("#replace_file").load("retribusi/Default/ReplaceFile");
