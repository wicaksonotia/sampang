...
class RiwayatController extends Controller {
    ...
    public function actionRiwayatListGrid() {
        ...
        $result1 = VRiwayat::model()->find($criteria);
        $id_kendaraan = 0;
        if (!empty($result1)) {
            $id_kendaraan = $result1->id_kendaraan;
        }
        $dataJson = array();
    ...
    public function actionViewImage() {
        ...
        $result = TblHasilUji::model()->findByPk($id_hasil_uji);
...
//        $sheet->getPageSetup()->setVerticalCentered(true);
        ...
        $sheet->setCellValue("A1", "UPTD PENGUJIAN KENDARAAN BERMOTOR");
        $sheet->getStyle("A1")->applyFromArray($styleBold12);
        $sheet->getStyle("A1")->applyFromArray($styleTengah);

        $sheet->mergeCells("A2:J2");
        $sheet->setCellValue("A2", "DINAS PERHUBUNGAN KAB. KOTAWARINGIN TIMUR");
        ...
        $sheet->setCellValue("H16", ": " . $data->perusahaan);
        ...
        $sheet->setCellValue("C24", ": Rp." . number_format($data->total));
        ...
        header('Content-Disposition: attachment;filename="riwayat_pemeriksaan[' . $data->no_kendaraan . '].xlsx"');
